...
use crate::{errors::DbError, models::{Article, Articles, Author, Blog, Category, CategoryType, Comment}};
...
pub async fn add_article(client: &Client, article: Article) -> Result<Article, DbError> {
	...
	client
		.query(&stmt, &[&article.title, &article.file_name, &article.date, &article.author_id, &article.blog_id])
		.await?
		.iter()
...
pub async fn add_author(client: &Client, author: Author) -> Result<Author, DbError> {
	...
	client
		.query(&stmt, &[&author.nickname, &author.name, &author.surname, &author.date, &author.email, &author.phone, &author.image, &author.birthday, &author.password])
		.await?
		.iter()
...
pub async fn add_comment(client: &Client, comment: Comment) -> Result<Comment, DbError> {
	...
	client
		.query(&stmt, &[&comment.article_id, &comment.author_id, &comment.text])
		.await?
		.iter()
